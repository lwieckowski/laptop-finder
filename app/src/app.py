...
def index():
    ...
    return render_template(
        "index.jinja", laptops=laptops, navlinks=NAVLINKS, filters=FILTERS, page=1, per_page=PER_PAGE, total=len(LAPTOPS)
    )
...
def filter():
    ...
    if not form_data:
        ...
        return render_template("features/table_body.jinja", laptops=laptops, page=page, per_page=PER_PAGE, total=len(LAPTOPS))
    ...
    return render_template("features/table_body.jinja", laptops=res_paged, page=page, per_page=PER_PAGE, total=len(res))
